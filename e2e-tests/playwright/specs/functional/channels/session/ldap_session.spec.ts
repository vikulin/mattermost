...
import {LoginPage, duration, expect, test} from '@mattermost/playwright-lib';
...
test.describe('LDAP session extension', () => {
    ...
    test(
        ...
        {tag: '@ldap'},
        async ({pw}) => {
            const {adminClient, channelsPage, ldapUser} = await setup(pw, true);
    ...
    test(
        ...
        async ({pw}) => {
            ...
            await expect(loginPage.title).toBeVisible({timeout: duration.half_min});

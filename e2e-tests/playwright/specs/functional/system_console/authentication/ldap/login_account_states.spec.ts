...
test.describe('LDAP authentication and guest filters', () => {
    ...
    test('LDAP login existing MM admin', {tag: '@ldap'}, async ({pw}) => {
        ...
        const channelsPage = new ChannelsPage(pw.loginPage.page);
        await channelsPage.expectChannelVisible('Town Square');
        await channelsPage.expectLoggedIn();
    });
    ...
    test('LDAP login, new MM user, no channels', {tag: '@ldap'}, async ({pw}) => {
        ...
        const channelsPage = new ChannelsPage(pw.loginPage.page);
        await channelsPage.expectTeamSelectionVisible();
    });
    ...
    test('LDAP login, new guest, no channels', {tag: '@ldap'}, async ({pw}) => {
        ...
        const channelsPage = new ChannelsPage(pw.loginPage.page);
        await channelsPage.expectGuestNoChannelsMessage();
    });
});

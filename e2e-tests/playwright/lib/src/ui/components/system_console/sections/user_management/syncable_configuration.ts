...
export default class SyncableConfiguration {
    ...
    async expectGroupVisible(groupDisplayName: string) {
        await expect(this.page.getByText(groupDisplayName, {exact: true})).toBeVisible({timeout: duration.half_min});
    }

    async expectGroupNotVisible(groupDisplayName: string) {
        await expect(this.page.getByText(groupDisplayName, {exact: true})).not.toBeVisible();
    }

    /**
     * Removes the first listed synchronized group without asserting the resulting empty state.
     * Use when other groups remain after removal.
     */
    async removeFirstGroup() {
        await this.page.getByRole('link', {name: 'Remove', exact: true}).first().click();
    }

    /**
     * Cancels the configuration edits and confirms discarding unsaved changes.
     */
    async discardUnsavedChanges() {
        await this.page.getByRole('link', {name: 'Cancel', exact: true}).click();
        await this.page.getByRole('button', {name: /Discard|Yes/}).click();
    }
}

...
export default class GroupConfiguration {
    ...
    async expectTeamRemovalWarning(teamDisplayName: string) {
        await expect(
            this.page.getByText(
                `Removing this membership will prevent future users in this group from being added to the ${teamDisplayName} team.`,
            ),
